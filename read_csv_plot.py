...
def plot_csv(folder_with_csv, path_to_csv, folder_to_save_plots, path_to_images=os.path.abspath('images_bpg')):
    ...
    df_read = pd.read_csv(full_path_to_csv, usecols=['image', 'QS', 'msssim'])
    ...
    for i in range(len(csvs)):
        ...
        print(new_df)
        lines = plt.plot(new_df.QS, new_df.msssim, label=csvs[i])
...
def run_plot():
    ...
    if os.path.isdir(folder_to_save_plots):
        ...
        print(f"Folder {folder_to_save_plots} created")
